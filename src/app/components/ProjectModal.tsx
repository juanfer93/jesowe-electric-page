...
export default function ProjectModal({ project, isOpen, onClose }: Props) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Content */}
      <div
        className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-2xl transform transition-all duration-300 ease-out ${
          ...
          {project?.gallery && project.gallery.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 justify-items-center">
              {project.gallery.map((src, index) => (
                <div
                  key={index}
                  className="relative h-56 sm:h-64 md:h-72 w-full max-w-[420px] rounded-xl overflow-hidden bg-slate-100 shadow-sm border border-slate-100"
                >
                  <Image
                    src={src}
